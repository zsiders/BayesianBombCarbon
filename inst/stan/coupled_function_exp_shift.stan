...
transformed parameters{
	real cdf_mu; //mean of the Gaussian CDF
	vector[Nref] C14_ref_hat; //predicted reference
	vector[Nobs] C14_obs_hat;  //predicted observations
	vector[Nobs] C14_obsadj_hat;  //predicted adj. observations
	...
	for(i in 1:Nref){
		C14_ref_hat[i] = lambda+kappa * exp((mu*gamma)+(sigmaN*gamma)/2) * exp(-gamma*BY_ref[i]) * normal_cdf(BY_ref[i] | cdf_mu, sigmaN);
	}
	for(i in 1:Nobs){
		C14_obs_hat[i] = lambda+kappa * exp((mu*gamma)+(sigmaN*gamma)/2) * exp(-gamma*BY_obs[i]) * normal_cdf(BY_obs[i] | cdf_mu, sigmaN);
		C14_obsadj_hat[i] = lambda+kappa * exp((mu*gamma)+(sigmaN*gamma)/2) * exp(-gamma*BY_adj[i]) * normal_cdf(BY_adj[i] | cdf_mu, sigmaN);
	}
...
generated quantities{
	...
	for(i in 1:Np){
		C14_pred_hat[i] = lambda+kappa * exp((mu*gamma)+(sigmaN*gamma)/2) * exp(-gamma*BY_pred[i]) * normal_cdf(BY_pred[i] | cdf_mu, sigmaN);

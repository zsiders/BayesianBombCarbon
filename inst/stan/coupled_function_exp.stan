...
model{
	...
	//model
		for(i in 1:Nref){
			C14_ref[i] ~ normal(C14_ref_hat[i], sigma_ref);
		}
}
generated quantities{
	...
	for(ii in 1:Np){
		C14_pred_hat[ii] = lambda+kappa * exp((mu*gamma)+(sigmaN*gamma)/2) * exp(-gamma*BY_pred[ii]) * normal_cdf(BY_pred[ii] | cdf_mu, sigmaN);

...
data_prep <- function(df_ref, df_unk, model = 'bspline', ll_wt = 5, pred.by=list(min.by=1940, max.by=2020, inc.by=1), adj_prior = NULL, bspline.control = list(knot.min = 10, knot.adj = 4, fixed.knot=NULL, spline.degree = 3, pad.spline=0.01)){
	...
	v <- list(flag = NULL, data = NULL, model = model)
	if(class(v$model)=='try-error') stop('model argument must be one of exponential, linear, or bspline')
	...
	if(v$model == 'bspline'){
		print('B-spline model indicated, adding additional data values for spline control')
		v$data$Nk <- pmax(bspline.control$knot.min,floor(v$data$Nref/bspline.control$knot.adj)) #can adjust knots with this
		if(!is.null(bspline.control$fixed.knot)){
			if(is.integer(bspline.control$fixed.knot)){
				v$data$Nk <- bspline.control$fixed.knot
			}else if(!is.integer(bspline.control$fixed.knot)){
